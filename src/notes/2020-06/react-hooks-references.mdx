...
Notice here that the `applicationMessages` argument is passed to the `React.useCallback` dependency array. The `loadApplicationMessages` callback function is then passed another React Hook, which loads the data asynchronously.
